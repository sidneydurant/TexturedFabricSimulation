...
public class Render {
	...
	// draws an axis aligned rectangle, with x,y being the centerpoint
	public void drawColoredRectangle( int x, int y, int w, int h, Color c ){
		
		int topLeftX = x - w/2;
		int topLeftY = y - h/2;
		
		// if the entire Rectangle is on the screen, drawUnchecked point
		if ( topLeftX>0 && topLeftX+w < WIDTH && topLeftY>0 && topLeftY+h < HEIGHT){
			for( int xi = topLeftX + w-1; xi >= topLeftX; xi-- ){
				for( int yi = topLeftY + h-1; yi >= topLeftY; yi-- ){
					drawUncheckedColoredPoint( xi, yi, c );
				}
			}
			
		}else{ // draw checked point
			for( int xi = topLeftX + w-1; xi >= topLeftX; xi-- ){
				for( int yi = topLeftY + h-1; yi >= topLeftY; yi-- ){
					drawColoredPoint( xi, yi, c );
				}
			}
		}
	}
	...
	public void drawColoredPoint(int x, int y, Color c) {
		if (x < WIDTH && x >= 0 && y < HEIGHT && y >= 0) {
			drawUncheckedColoredPoint( x, y, c );
		}
	}
