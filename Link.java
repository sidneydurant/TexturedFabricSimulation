public class Link {
	...
	public boolean solve() {
		...
		float difference = linearDist >= 1 ? (restingDistance - linearDist)
				/ linearDist : 1; // 1 is a fallback if dist < 1

		float translateX = distX * .6f * difference;
		float translateY = distY * .6f * difference;

		// if one point is pinned, the other needs to translate more to stay in
		// line
	...
	public void draw(Render r) {
		r.drawColoredLine((int) p0.x, (int) p0.y, p0.c, (int) p1.x, (int) p1.y,
				p1.c);
	}
